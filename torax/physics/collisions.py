...
def coll_exchange(
  ...
  lambda_ei = _calculate_lambda_ei(
      core_profiles.temp_el.value, core_profiles.n_e.value * nref
  )
  ...
  log_Qei_coef = (
      jnp.log(Qei_mult * 1.5 * core_profiles.n_e.value * nref)
...
def calc_nu_star(
  ...
  lambda_ei_face = _calculate_lambda_ei(
      core_profiles.temp_el.face_value(), core_profiles.n_e.face_value() * nref
  )

  # ion_electron collisionality
  log_tau_e_Z1 = _calculate_log_tau_e_Z1(
      core_profiles.temp_el.face_value(),
      core_profiles.n_e.face_value() * nref,
      lambda_ei_face,
  )
