...
class SimulationStepFn:
  ...
  def __call__(
    ...
    # If sawtooth_just_crashed is True, then a normal PDE step will be carried
    # out. where a ToraxSimState will be returned with the default
    # sawtooth_just_crashed value of False.
    if (
        (self.mhd_models.sawtooth is not None)
        and (not input_state.sawtooth_just_crashed)
    ):
      ...
      output_state = dataclasses.replace(
          output_state,
          sawtooth_just_crashed=sawtooth_triggered,
      )
