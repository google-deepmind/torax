...
class GasPuffFeedbackSourceTest(test_lib.SingleProfileSourceTestCase):
  ...
  def test_feedback_mode(self):
    ...
    config['sources']['gas_puff']['target_average_n_e'] = float(
        initial_line_avg + 1e19
    )
    config['sources']['gas_puff']['feedback_gain'] = 10.0
    config['sources']['gas_puff']['S_feedforward'] = 0.0
    torax_config = model_config.ToraxConfig.from_dict(config)
    ...
    assert isinstance(gas_puff_params, gas_puff_feedback_source.RuntimeParams)
    chex.assert_trees_all_close(
        gas_puff_params.target_average_n_e, float(initial_line_avg + 1e19)
    )
