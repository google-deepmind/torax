...
# Boolean array indicating whether the sawtooth model just crashed.
SAWTOOTH_JUST_CRASHED = "sawtooth_just_crashed"
...
class StateHistory:
  ...
  def __init__(
    ...
    self.sawtooth_just_crashed = np.array(
        [state.sawtooth_just_crashed for state in state_history]
    )
  ...
  def simulation_output_to_xr(
    ...
    Returns:
      ...
      datasets. The top level dataset contains the following variables:
        ...
        - sawtooth_just_crashed: Time-series boolean indicating whether the
            state corresponds to post-sawtooth-crash.
    ...
    top_level_xr_dict = {
        ...
        SAWTOOTH_JUST_CRASHED: xr.DataArray(
            self.sawtooth_just_crashed, dims=[TIME], name=SAWTOOTH_JUST_CRASHED
        ),
    }
