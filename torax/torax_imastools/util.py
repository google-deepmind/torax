...
def core_profiles_to_IMAS(ids, state, geometry):
  ...
  ids.global_quantities.current_non_inductive[0] = state.currents.I_bootstrap
  ids.profiles_1d[0].grid.rho_tor_norm = geometry.rho_norm
  ids.profiles_1d[0].grid.rho_tor = geometry.rho
  ids.profiles_1d[0].grid.psi = state.psi.value
  ids.profiles_1d[0].grid.volume = geometry.volume
  ids.profiles_1d[0].grid.area = geometry.area
  ids.profiles_1d[0].electrons.temperature = state.temp_el.value
  ids.profiles_1d[0].electrons.density = state.ne.value
  ids.profiles_1d[0].ion[0].z_ion = state.Zi
  ids.profiles_1d[0].ion[0].temperature = state.temp_ion.value
  ids.profiles_1d[0].ion[0].density = state.ni.value
  # assume no molecules, revisit later
  ids.profiles_1d[0].ion[0].element[0].a = state.Zi
  ids.profiles_1d[0].ion[0].element[0].z_n = state.Ai
  ids.profiles_1d[0].ion[1].z_ion = state.Zimp
  ids.profiles_1d[0].ion[1].temperature = state.temp_ion.value
  ids.profiles_1d[0].ion[1].density = state.nimp.value
  # assume no molecules, revisit later
  ids.profiles_1d[0].ion[1].element[0].a = state.Zimp
  ids.profiles_1d[0].ion[1].element[0].z_n = state.Aimp
  ids.profiles_1d[0].q = face_to_cell(state.q_face)
  ids.profiles_1d[0].magnetic_shear = face_to_cell(state.s_face)
  ids.profiles_1d[0].j_total = state.currents.jtot
  ids.profiles_1d[0].j_ohmic = state.currents.johm
  ids.profiles_1d[0].j_non_inductive = state.currents.generic_current_source
  ids.profiles_1d[0].j_bootstrap = state.currents.j_bootstrap
  ids.profiles_1d[0].conductivity_parallel = state.currents.sigma
  # ids. = state.psidot.value
  # ids. = face_to_cell(state.currents.jtot_face)
  # ids. = face_to_cell(state.currents.Ip_profile_face)
  # ids. = face_to_cell(state.currents.j_bootstrap_face)
  return ids
