...
class Linear(torax_pydantic.BaseModelMutable):
  ...
  d_per: pydantic.PositiveFloat = 10.0
...
class OptimizerThetaMethodConfig(Linear):
...
class NewtonRaphsonThetaMethodConfig(Linear):
...
StepperConfig = Union[
    Annotated[Linear, pydantic.Tag('linear')],
    Annotated[NewtonRaphsonThetaMethodConfig, pydantic.Tag('newton_raphson')],
    Annotated[
        OptimizerThetaMethodConfig,
        pydantic.Tag('optimizer'),
    ],
]
