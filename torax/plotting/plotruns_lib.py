...
class FigureProperties:
  ...
  def needs_slider(self) -> bool:
    """Checks if any plot needs a time slider."""
    return any(
...
def plot_run(
  ...
  # Do not create the slider if not needed.
  if plot_config.needs_slider():
...
def create_figure(plot_config: FigureProperties):
  ...
  if plot_config.needs_slider():
    # Add an extra smaller row for the slider
