...
class FlattenProfileTest(parameterized.TestCase):
  ...
  def test_psi_redistribution_with_current_flattening(self):
    ...
    original_jtot_value, _, _ = psi_calculations.calc_jtot(
    ...
    redistributed_psi_profile = flatten_profile.redistribute_psi_profile(
        rho_norm_q1=jnp.array(rho_norm_q1),
        rho_norm_mixing=jnp.array(rho_norm_mixing),
        flattening_factor=jnp.array(flattening_factor),
        original_psi_profile=original_psi_profile,
        original_jtot_value=original_jtot_value,
        Ip_total=Ip,
        geo=self.geo,
    )

    redistributed_jtot_value, _, _ = psi_calculations.calc_jtot(
