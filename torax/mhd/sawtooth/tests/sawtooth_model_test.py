...
class SawtoothModelTest(parameterized.TestCase):
  ...
  def test_sawtooth_crash(self):
    ...
    np.testing.assert_equal(output_state.sawtooth_just_crashed, np.array(True))
    ...
    np.testing.assert_allclose(
        output_state.core_profiles.temp_el.value, _POST_CRASH_T, rtol=1e-6
    )
    np.testing.assert_allclose(
  ...
  def test_no_sawtooth_crash(self):
    ...
    np.testing.assert_equal(sim_error, state.SimError.NO_ERROR)
    np.testing.assert_equal(output_state.sawtooth_just_crashed, np.array(False))
  ...
  def test_no_subsequent_sawtooth_crashes(self):
    ...
    # q is in trigger condition, but sawtooth_just_crashed is True so no crash.
    ...
    # Check that the sawtooth is indeed triggered if sawtooth_just_crashed is
    # set to False.
    ...
    with self.subTest('no_subsequent_sawtooth_crashes'):
      ...
      np.testing.assert_equal(sim_error, state.SimError.NO_ERROR)
      np.testing.assert_equal(
          output_state_should_not_crash.sawtooth_just_crashed, np.array(False)
      )
      np.testing.assert_equal(
    ...
    with self.subTest('crashes_if_sawtooth_just_crashed_is_false'):
      ...
      np.testing.assert_equal(sim_error, state.SimError.NO_ERROR)
      np.testing.assert_equal(
          output_state_should_crash.sawtooth_just_crashed, np.array(True)
      )
      np.testing.assert_equal(
