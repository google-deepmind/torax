...
class SawtoothModel(stepper.Solver):
  ...
  def _x_new(
    ...
    following a short (configurable) dt is returned, with core_profiles further
    modified by psidot assumed at time t, the new boundary conditions, and
    sources, transport, geo, and pedestal outputs consistent with the new
    core_profiles at time t_plus_crash_dt.
    ...
    Returns:
      Updated CoreProfiles tuple and other updated ToraxSimState attributes.
    """
    ...
    def _make_redistributed_state_and_post_processed_outputs() -> tuple[
        tuple[cell_variable.CellVariable, ...],
        source_profiles_lib.SourceProfiles,
        state.CoreTransport,
        state.SolverNumericOutputs,
