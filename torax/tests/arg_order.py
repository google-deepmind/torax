        ...
        for param in params:
          if param not in self.counts:
            ...
            if self.counts[param] >= self.THRESHOLD:
              if param not in sim.CANONICAL_ORDER:
                for other in sim.CANONICAL_ORDER:
                  print(param, other, param == other)
                raise TypeError(
                    "Common param not in canonical order: param "
                    f'"{param}" of {module.__name__}.{name}'
                )
        params = [param for param in params if param in sim.CANONICAL_ORDER]
        print("\t\tFiltered params: ", params)
        idxs = [sim.CANONICAL_ORDER.index(param) for param in params]
        print("\t\tidxs: ", idxs)
