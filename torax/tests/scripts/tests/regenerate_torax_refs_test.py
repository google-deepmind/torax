...
class RegenerateRefsTest(parameterized.TestCase):
  ...
  def test_no_summary_flag_suppresses_output(self):
    ...
    with mock.patch('sys.stdout', new_callable=io.StringIO) as mock_stdout:
      ...
      self.assertIn('Regenerating references for: circular_references', output)
      self.assertNotIn('Full data for circular_references', output)

  @flagsaver.flagsaver(case=['circular_references'], no_summary=False)
  def test_summary_is_printed_by_default(self):
    ...
    with mock.patch('sys.stdout', new_callable=io.StringIO) as mock_stdout:
      ...
      output = mock_stdout.getvalue()
      self.assertIn('Regenerating references for: circular_references', output)
      self.assertIn('Full data for circular_references', output)
      self.assertIn('circular_references.psi', output)
