...
if 'output_dir' not in FLAGS:
  flags.DEFINE_string(
      ...
      ' the default in-tree location will be used.',
  )

_JSON_FILENAME = 'references.json'
...
class NumpyEncoder(json.JSONEncoder):
  ...
  # Disable warning about obj renaming o in the parent class. obj is better.
...
def _calculate_new_references(
  ...
  if not isinstance(geo, standard_geometry.StandardGeometry):
    ...
    j_total_hires = initialization._get_j_total_hires(  # pylint: disable=protected-access
...
def _print_full_summary(case_name: str, new_values: dict[str, np.ndarray]):
  ...
  print(f'  Full data for {case_name}:')
...
def main(argv: Sequence[str]) -> None:
  ...
  cases_to_run = FLAGS.case or torax_refs.REFERENCES_REGISTRY.keys()
  if FLAGS.output_dir:
    output_dir = pathlib.Path(FLAGS.output_dir).expanduser()
    output_path = output_dir / _JSON_FILENAME
  else:
  ...
  if FLAGS.write_to_file and os.path.exists(output_path):
    print(f'Loading existing data from {output_path} to update...')
  ...
  for case_name in cases_to_run:
    ...
    print(f'Regenerating references for: {case_name}...')
    ...
    if not FLAGS.no_summary:
      _print_full_summary(case_name, new_values)

  if FLAGS.write_to_file:
    print(f'Writing all regenerated data to {output_path}...')
    with open(output_path, 'w') as f:
      json.dump(all_data, f, indent=2, cls=NumpyEncoder)
    print('Done.')
  else:
    print('Finished dry run. Not writing to file. Use --write_to_file to save.')
