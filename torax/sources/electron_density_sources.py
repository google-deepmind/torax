...
@dataclasses.dataclass(kw_only=True)
class GenericParticleRuntimeParams(runtime_params_lib.RuntimeParams):
  ...
  def make_provider(
      self,
      torax_mesh: geometry.Grid1D | None = None,
  ) -> GenericParticleRuntimeParamsProvider:
    return GenericParticleRuntimeParamsProvider(
        **self.get_provider_kwargs(torax_mesh)
    )


@chex.dataclass
class GenericParticleRuntimeParamsProvider(
    runtime_params_lib.RuntimeParamsProvider
):
  """Provides runtime parameters for a given time and geometry."""

  runtime_params_config: GenericParticleRuntimeParams
