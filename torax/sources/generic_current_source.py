...
class DynamicRuntimeParams(runtime_params_lib.DynamicRuntimeParams):
  ...
  def sanity_check(self):
    ...
    jax_utils.error_if_negative(self.width, 'width')
...
def calculate_generic_current(
  ...
  generic_current_form = jnp.exp(
      -((geo.rho_norm - dynamic_source_runtime_params.location) ** 2)
      / (2 * dynamic_source_runtime_params.width**2)
  )
...
class GenericCurrentSourceConfig(source_base.SourceModelBase):
  ...
  Attributes:
    ...
    width: width of "external" Gaussian current profile
    location: normalized radius of "external" Gaussian current profile
