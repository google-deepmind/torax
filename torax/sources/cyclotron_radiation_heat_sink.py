...
class RuntimeParams(runtime_params_lib.RuntimeParams):
  ...
  wall_reflection_coeff: runtime_params_lib.TimeInterpolatedInput = 0.9
  beta_min: float = 0.5
  beta_max: float = 8.0
...
def _alpha_closed_form(
  ...
  See _cyclotron_radiation_albajar for more details.
  ...
  Where ne_edge = 0 above. This fits are from the Albajar paper.
...
def _loss_for_beta_t(
    ...
    te_edge_value: array_typing.ScalarFloat,
) -> array_typing.ScalarFloat:
  ...
  Args:
    ...
    rho_norm: Normalized toroidal flux coordinate.
    te_data: The temperature data to be fit.
    te_edge_value: The value of temperature at the edge to use.
  ...
  alpha_t = _alpha_closed_form(
      ...
      profile_edge_value=te_edge_value,
      rho_norm=rho_norm,
      beta=beta_t,
  )
  return _te_loss_fn(
...
def _te_pred_fn(
  ...
  return (te_data[0] - te_edge_value) * (
      (1 - rho_norm**beta_t)
  ) ** alpha_t + te_data[-1]
...
def _solve_alpha_t_beta_t_grid_search(
  ...
  losses = jax.vmap(_loss_for_beta_t, in_axes=(0, None, None, None))(
      beta_t_trials,
      rho_norm,
      te_data,
      te_edge_value,
...
def cyclotron_radiation_albajar(
  ...
  alpha_t, beta_t = _solve_alpha_t_beta_t_grid_search(
      ...
      te_edge_value=core_profiles.temp_el.face_value()[-1],
