...
class UpdatersTest(parameterized.TestCase):
  ...
  def test_update_vloop_lcfs_from_psi(self):
    """Test consistency of the _update_vloop_lcfs_from_psi function."""
    ...
    core_profiles_t = mock.create_autospec(
        state.CoreProfiles, instance=True, psi=psi_t
    )

    vloop_lcfs_t_plus_dt = updaters._update_vloop_lcfs_from_psi(
        core_profiles_t, psi_t_plus_dt, dt
    )
