...
class ToraxConfig(torax_pydantic.BaseModelFrozen):
  ...
  def _defaults(cls, data: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        'internal_boundary_conditions' in configurable_data
        and isinstance(configurable_data['internal_boundary_conditions'], dict)
        and 'model_name'
        not in configurable_data['internal_boundary_conditions']
    ):
      configurable_data['internal_boundary_conditions'][
          'model_name'
      ] = 'prescribed'
    return configurable_data
