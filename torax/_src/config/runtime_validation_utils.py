...
def check_q_profile(config) -> None:
  """Checks if the q-profile is unphysically low and raises a warning."""

  ip = config.profile_conditions.Ip.value
  bt = config.geometry.geometry_configs.config.B_0
  a = config.geometry.geometry_configs.config.a_minor
  r0 = config.geometry.geometry_configs.config.R_major

  # Handle time-varying current
  if hasattr(ip, "__len__"):
      ip_val = float(ip[0])
  else:
      ip_val = float(ip)

  q_estimate = (bt * a**2) / (ip_val * r0)
...
def check_source_densities(config) -> None:
  """Checks for unphysically high source densities and raises warnings."""

  power = config.sources.power_source_density.value
  if power is not None and float(power) > 1e8:
      logging.warning("Power source density is very high (value=%s).", power)

  particle = config.sources.particle_source_density.value
  if particle is not None and float(particle) > 1e20:
      logging.warning("Particle source density is very high (value=%s).", particle)
