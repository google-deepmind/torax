...
class PlasmaComposition(torax_pydantic.BaseModelFrozen):
  ...
  Attributes:
    ...
    impurity: Impurity species. This is a dictionary that configures the
      impurity ions in the plasma. It has the following keys:
      `impurity_mode` (str): Determines how the impurity species are defined.
        Currently, only 'fractions' is implemented.
        *   `'fractions'`: The impurities are defined by their fractional
            concentrations within the total impurity population.
        *   `'n_e_ratios'`: The impurities are defined by their ratios of
            impurity to electron density. Z_eff is then an emergent quantity.
      `species` (dict): A dictionary mapping ion symbols (e.g., 'Ne', 'W') to
        their respective values. The interpretation of these values depends on
        the `impurity_mode`.
        *   If `impurity_mode` is `'fractions'`, the values represent the
            fraction of each ion species within the total impurity density.
            These fractions must sum to 1.0.
        *   If `impurity_mode` is `'n_e_ratios'`, the values represent the ratio
            of each ion species to the total electron density.
      ...
      `A_override` (TimeVaryingScalar | None): Optional. If provided, this value
        overrides the calculated average mass (A) of the impurity mixture.
      Other modes (`'n_e_density_ratios'` and `'n_e_density_ratios_and_Z_eff'`)
      are not yet implemented.
      Finally, backwards compatibility is provided for legacy inputs to
      `'impurity'`, e.g. string or dict inputs similar to `main_ion`, such as
      `'Ar'` or `{'Ar': 0.6, 'Ne': 0.4}`. In these cases, `impurity_mode` is
      inferred to be `'fractions'`, and `Z_override` and `A_override` are set to
      the top-level `Z_impurity_override` and `A_impurity_override` if provided,
      or `None` otherwise. A pydantic before validator handles this.
