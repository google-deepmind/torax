...
class NewtonRaphsonSolveBlockTest(parameterized.TestCase):
  ...
  def test_root_newton_raphson_basic(self):
    ...
    b = 1.0
    tol = 1e-9
    f_closed = functools.partial(test_function, a=a, b=b)
    ...
    def loss(x, a, b):
      root = jax_root_finding.root_newton_raphson(
          functools.partial(test_function, a=a, b=b), x, tol=tol
      )[0]
      return jnp.sum(root**2)

    f_grad = jax.grad(loss, argnums=(0, 1, 2))
    ...
    x_grad, a_grad, b_grad = f_grad(x_init, a, b)

    with self.subTest('gradient_correctness'):
      chex.assert_trees_all_equal(x_grad, jnp.array([0.0, 0.0], dtype=dtype))
      chex.assert_trees_all_close(a_grad, a_grad_diff, atol=1e-3)
      chex.assert_trees_all_close(b_grad, b_grad_diff, atol=1e-3)
