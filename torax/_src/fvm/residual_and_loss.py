...
def apply_internal_boundary_conditions(
  ...
  For each (cell, channel) where ibc_mask is True, replaces the PDE row in the
  matrix equation with the algebraic constraint:
  ...
  Args:
    ...
    ibc_mask: Boolean array, shape (num_cells, num_channels). True at
      constrained (cell, channel) entries.
    ibc_target_vec: Float array, shape (num_cells, num_channels). Target values
      in solver-scaled units.
  ...
  # diagonal has shape (N, C, C). ibc_mask[:, :, None] broadcasts over the
  # last axis so that row j of block i is replaced by eye[j, :] when
  # ibc_mask[i, j] is True. This makes L · x_new = 1 · x_new at that entry.
  new_lhs_diag = jnp.where(
      ibc_mask[:, :, None], eye[None, :, :], lhs.diagonal
  )
  ...
  new_lhs_lower = jnp.where(ibc_mask[1:, :, None], 0.0, lhs.lower)
  # upper has shape (N-1, C, C): block i lives in cell i's equation, so we
  # index the mask at [:-1] (cells 0..N-2).
  new_lhs_upper = jnp.where(ibc_mask[:-1, :, None], 0.0, lhs.upper)
  # Zero the LHS source term so the constraint has no additive forcing on the
  # left-hand side.
  new_lhs_vec = jnp.where(ibc_mask, 0.0, lhs_vec)

  # --- RHS: zero the matrix (decouple x_old) and inject the target ---
  # All RHS matrix bands are zeroed at constrained rows so x_old does not
  # enter the constraint equation.
  new_rhs_diag = jnp.where(ibc_mask[:, :, None], 0.0, rhs.diagonal)
  new_rhs_lower = jnp.where(ibc_mask[1:, :, None], 0.0, rhs.lower)
  new_rhs_upper = jnp.where(ibc_mask[:-1, :, None], 0.0, rhs.upper)
  # The RHS forcing is replaced with the target value, giving
  # the final constraint: 1 · x_new[i, j] = target[i, j].
  new_rhs_vec = jnp.where(ibc_mask, ibc_target_vec, rhs_vec)
