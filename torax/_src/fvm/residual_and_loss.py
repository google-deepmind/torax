...
def theta_method_matrix_equation(
  ...
  Returns:
    A tuple of (lhs, lhs_vec, rhs, rhs_vec) where:
      lhs: BlockTriDiagonal for the LHS matrix A.
      lhs_vec: LHS vector a.
      rhs: BlockTriDiagonal for the RHS matrix B.
  ...
  c_new_matrix, c_new = discrete_system.calc_c(
      x_new_guess,
      coeffs_new,
      convection_dirichlet_mode,
      convection_neumann_mode,
  ...
  if theta_exp > 0.0:
    ...
    c_old_matrix, c_old = discrete_system.calc_c(
        x_old,
        coeffs_old,
        convection_dirichlet_mode,
        convection_neumann_mode,
    ...
    rhs_vec = scale_old * c_old
  else:
    rhs = tridiagonal.BlockTriDiagonal.from_diagonal(tc_in_old / tc_in_new)
    rhs_vec = jnp.zeros((rhs.num_blocks, rhs.block_size), dtype=tc_in_new.dtype)

  return lhs, lhs_vec, rhs, rhs_vec
...
def theta_method_block_residual(
  ...
  x_old_array = jnp.stack([var.value for var in x_old], axis=-1)
