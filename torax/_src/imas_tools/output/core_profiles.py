...
def core_profiles_to_IMAS(
  ...
  for i in range(len(times)):
    ...
    ids.profiles_1d[i].grid.psi_magnetic_axis = cp_state.psi._left_face_value()[
        0
    ]
    ...
    for ion in range(len(impurities)):
      ...
      ion_properties = constants.ION_PROPERTIES_DICT[symbol]
      ids.profiles_1d[i].ion[num_of_main_ions + ion].z_ion_1d = (
          Z_avg_per_species[symbol]
      )
      try:
        ids.profiles_1d[i].ion[num_of_main_ions + ion].name = symbol
      except AttributeError:
        # TODO(b/459479939): i/539) - Indicate supported dd_versions and switch on
        # that instead of using a try-except.
        # Case ids is plasma_profiles in early DDv4 releases.
        ids.profiles_1d[i].ion[num_of_main_ions + ion].label = symbol
      ids.profiles_1d[i].ion[
          num_of_main_ions + ion
      ].temperature = cp_state.T_i.cell_plus_boundaries()
      ids.profiles_1d[i].ion[num_of_main_ions + ion].density = (
          cp_state.n_impurity.cell_plus_boundaries() * frac
      )
      ids.profiles_1d[i].ion[num_of_main_ions + ion].density_thermal = (
          cp_state.n_impurity.cell_plus_boundaries() * frac
      )
      ids.profiles_1d[i].ion[num_of_main_ions + ion].density_fast = np.zeros(
          len(ids.profiles_1d[i].grid.rho_tor_norm)
      )
      # Proportion of this ion for pressure ratio computation.
      total_ions_mixture_fraction = (
      ...
      ids.profiles_1d[i].ion[num_of_main_ions + ion].pressure = (
          total_ions_mixture_fraction
          * cp_state.pressure_thermal_i.cell_plus_boundaries()
      )
      ids.profiles_1d[i].ion[num_of_main_ions + ion].pressure_thermal = (
          total_ions_mixture_fraction
          * cp_state.pressure_thermal_i.cell_plus_boundaries()
      )
      ids.profiles_1d[i].ion[num_of_main_ions + ion].element.resize(1)
      ids.profiles_1d[i].ion[num_of_main_ions + ion].element[
          0
      ].a = ion_properties.A
      ids.profiles_1d[i].ion[num_of_main_ions + ion].element[
          0
      ...
      ids.global_quantities.ion[num_of_main_ions + ion].t_i_volume_average[
          i
      ] = (
          post_processed_outputs_slice.T_i_volume_avg * 1e3
