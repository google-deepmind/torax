...
def _fill_global_quantities(
  ...
  for ion in range(num_ions):
    ids.global_quantities.ion[ion].t_i_volume_average.resize(
        len(times), refcheck=False
    )
    ids.global_quantities.ion[ion].n_i_volume_average.resize(
        len(times), refcheck=False
    )
...
# TODO(b/323504363): b/424190851 - Map impurity quantities in PostProcessedOutputs on
# cell_plus_boundaries grid to get rid of this function.
...
def _fill_profiles_1d_currents(
  ...
  """Fills `currents` section of `profiles_1d` in-place for the IDS."""
  ...
  j_parallel_total_face = math_utils.cell_to_face(
  ...
  j_ohmic_face = math_utils.cell_to_face(
  ...
  j_non_inductive_face = math_utils.cell_to_face(
...
def _fill_profiles_1d_ions(
  ...
  # TODO(b/323504363): b/459479939 - i/2212 Fix extrapolation once we do multi-ion transport.
  ...
  impurity_density_scaling, Z_per_species = (
...
def _fill_main_ions(
    ...
    symbol: str,
    frac: float,
    T_i: jt.Float[jax.Array, 'cell+2'],
    n_i: jt.Float[jax.Array, 'cell+2'],
    n_impurity_true: jt.Float[jax.Array, 'cell+2'],
    pressure_thermal_i: jt.Float[jax.Array, 'cell+2'],
    post_processed_outputs_slice: post_processing.PostProcessedOutputs,
) -> None:
  ...
  Args:
      ...
      i: The time index.
      ion: The ion index.
      symbol: The symbol of the ion.
      frac: The fraction of the ion.
      T_i: The ion temperature.
      n_i: The ion density.
      n_impurity_true: The scaled bulk impurity density.
      pressure_thermal_i: The ion thermal pressure.
      post_processed_outputs_slice: The post processed outputs slice.
  ...
  n_species = n_i * frac  # Density of this species.
