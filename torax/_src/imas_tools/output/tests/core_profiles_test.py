...
class CoreProfilesTest(sim_test_case.SimTestCase):
  ...
  def test_round_trip_IMAS_TORAX_IMAS(
    ...
    for i in range(len(initial_core_profiles.profiles_1d)):
      ...
      initial_core_profiles.profiles_1d[i].ion[0].density = (
          0.48 * initial_core_profiles.profiles_1d[i].electrons.density
      )
      initial_core_profiles.profiles_1d[i].ion[1].density = (
          0.48 * initial_core_profiles.profiles_1d[i].electrons.density
      )
      initial_core_profiles.profiles_1d[i].ion[2].density = (
          0.02 * initial_core_profiles.profiles_1d[i].electrons.density
      )
      ...
      initial_core_profiles.profiles_1d[i].n_i_total_over_n_e = np.sum(
          [
              initial_core_profiles.profiles_1d[i].ion[j].density
              / initial_core_profiles.profiles_1d[i].electrons.density
              for j in range(3)
          ],
          axis=0,
      )
    ...
    np.testing.assert_allclose(
        ...
        np.interp(
            rhon_in,
            rhon_out,
            torax_output_core_profiles_ids.profiles_1d[0].n_i_total_over_n_e,
        ),
        rtol=rtol,
        atol=atol,
    ...
    np.testing.assert_allclose(
        ...
        np.interp(
            rhon_in,
            rhon_out,
            torax_output_core_profiles_ids.profiles_1d[0].zeff,
        ),
        rtol=rtol,
        atol=atol,
