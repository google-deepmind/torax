...
from torax.tests import sim_test
...
class CoreProfilesTest(sim_test_case.SimTestCase):
  ...
  def test_round_trip_with_IMAS_profiles(
    ...
    """Test that TORAX simulation can be ran from a standard config, saved into
    an IDS and ran again from the IDS data profiles without changes.
    The test:
    ...
    ref_profiles, ref_time = self._get_refs(
        'test_iterhybrid_predictor_corrector.nc', sim_test._ALL_PROFILES
    )
