...
def imas_to_torax_xr(
  ...
  """
  logging.warning(
      'Loading IMAS data is currently experimental. Only the variables'
      ' necessary for the defaul TORAX plot config are loaded.'
  )
  try:
  ...
  # TODO(b/500974235): Investigate IMAS definition of grid to ensure that it is
  # cell+boundaries, which is what we are assuming here.
  ...
  # TODO(b/500974235): this is not necessarily true if the grid is non-uniform.
  ...
  def _get_core_profile(path):
    return np.asarray([
        validation.get_nested_attr(p, path)
        for p in core_profiles_ids.profiles_1d
    ])
  ...
  def _get_source_global_quantity(source_name, path):
    source_idx = source_name_to_idx_mapping[source_name]
    return np.asarray([
        validation.get_nested_attr(g, path)
        for g in core_sources_ids.source[source_idx].global_quantities
    ])

  def _get_equilibrium(path):
    return np.asarray([
        validation.get_nested_attr(t, path) for t in equilibrium_ids.time_slice
    ])

  def _get_summary(path):
    return np.asarray(validation.get_nested_attr(summary_ids, path).value)
  ...
  # Not supported values that need to be populated in order for the plotting
  # library to work.
