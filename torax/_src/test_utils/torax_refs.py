...
_JSON_PATH = (
    config_loader.torax_path() / '_src' / 'test_utils' / 'references.json'
)
try:
  with open(_JSON_PATH, 'r') as f:
    _ALL_REFERENCES = json.load(f)
except FileNotFoundError:
  raise FileNotFoundError(
      f'Reference data file not found at {_JSON_PATH}.'
  ) from None
...
def _build_references_from_case_data(
  ...
  case_data = _ALL_REFERENCES[case_name]
