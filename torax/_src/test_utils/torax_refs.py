...
def sawtooth_references() -> References:
  """Reference values for sawtooth model tests.
  
  These references include post-crash profiles generated by running
  a simulation step with sawtooth crash. They can be regenerated with:
    python -m torax.tests.scripts.regenerate_torax_refs \\
        --case=sawtooth_references --write_to_file
  """
  _NRHO = 10
  ...
  if 'sawtooth_references' not in all_references:
      raise ValueError(
          "sawtooth_references not found in references.json. "
          "Generate them with: python -m torax.tests.scripts.regenerate_torax_refs "
          "--case=sawtooth_references --write_to_file"
      )
  
  case_data = all_references['sawtooth_references']
  
  psi_value = np.array(case_data['psi'])
  psi_face_grad = np.array(case_data['psi_face_grad'])
  psi = cell_variable.CellVariable(
      value=psi_value,
      right_face_grad_constraint=psi_face_grad[-1] if len(psi_face_grad) > 0 else 0.0,
      dr=geo.drho_norm,
  )
...
REFERENCES_REGISTRY: Final[Mapping[str, Callable[[], References]]] = (
    immutabledict.immutabledict({
        ...
        'sawtooth_references': sawtooth_references,
    })
)
