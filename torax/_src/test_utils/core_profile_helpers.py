...
def verify_core_profiles(
  ...
  np.testing.assert_allclose(
      ...
      ref_profiles[output.J_TOR_TOTAL][index, 0],
      err_msg=(
          "Mismatch for j_total_face[0]. Core profile:"
          f" {core_profiles.j_total_face[0]}, Ref profile ({output.J_TOR_TOTAL}"
          f" start val): {ref_profiles[output.J_TOR_TOTAL][index, 0]}"
      ),
  )
  np.testing.assert_allclose(
      core_profiles.j_total_face[-1],
      ref_profiles[output.J_TOR_TOTAL][index, -1],
      err_msg=(
          "Mismatch for j_total_face[-1]. Core profile:"
          f" {core_profiles.j_total_face[-1]}, Ref profile"
          f" ({output.J_TOR_TOTAL} end val):"
          f" {ref_profiles[output.J_TOR_TOTAL][index, -1]}"
      ),
  )
  np.testing.assert_allclose(
