...
class StepFunctionTest(parameterized.TestCase):
  ...
  def test_fixed_step_with_high_density_errors_and_does_not_hang(self):
    ...
    # time.
    # We force the min_dt to be reached in the substeps by 1) reducing
    # min_dt and 2) selecting a case with difficult dynamics, so that a small
    # dt is required for a long time.
    ...
    torax_config.update_fields({'numerics.min_dt': 1e-3})
