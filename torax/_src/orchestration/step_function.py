...
class SimulationStepFn:
  ...
  def __call__(
    ...
    Args:
      ...
      max_dt: The maximum time step duration to allow the simulation to take. If
        set this will override the properities set in the numerics config. If
  ...
  def fixed_time_step(
    ...
    """Runs the simulation until it has advanced by numerics.fixed_dt."""
    remaining_dt = jnp.asarray(
        self.runtime_params_provider.numerics.fixed_dt, jax_utils.get_dtype()
    )
