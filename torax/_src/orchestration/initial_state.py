...
def _get_initial_state(
  ...
  if runtime_params.pedestal.use_formation_model_with_adaptive_source:
    # Call the pedestal model to get the dynamically computed rho_norm_ped_top.
    # This ensures models like EPEDNN that compute the pedestal top location
    # are properly supported from initialization.
    pedestal_model_output = physics_models.pedestal_model(
        runtime_params, geo, initial_core_profiles, initial_core_sources,
    )
    rho_norm_ped_top = pedestal_model_output.rho_norm_ped_top
