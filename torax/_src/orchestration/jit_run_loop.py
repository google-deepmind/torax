...
def run_loop_jit(
  ...
  initial_dynamic_runtime_params_slice = (
      step_fn.dynamic_runtime_params_slice_provider(initial_state.t)
  )
  ...
  def _cond_fun(inputs):
    ...
    not_done = step_fn.time_step_calculator.not_done(
        current_state.t,
        step_fn.dynamic_runtime_params_slice_provider.numerics.t_final,
        time_step_calculator_dynamic_params,
    )
...
def _unstack_pytree_history(
  ...
  Args:
    states_history: A PyTree where each leaf is a JAX array with
                    shape (max_steps + 1, ...) representing the history
                    of that component over time.
    post_processed_outputs_history: A PyTree where each leaf is a JAX array
      with shape (max_steps + 1, ...) representing the history of that
      component over time.
...
def run_loop(
  ...
  numerics = dynamic_runtime_params_slice_provider.numerics
  max_steps = int(
