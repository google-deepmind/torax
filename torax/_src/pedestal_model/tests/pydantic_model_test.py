...
class PedestalModelPydanticTest(parameterized.TestCase):
  ...
  def test_source_mode_validation(self):
    ...
    # This should be fine (default mode is ADAPTIVE_SOURCE)
    pydantic_model.SetTpedNped.from_dict(
        {"use_formation_model_with_adaptive_source": True}
    )

    # This should fail because mode is ADAPTIVE_TRANSPORT but use_formation
    # model_with_adaptive_source is True
  ...
  def test_transition_time_width_must_be_positive(self):
