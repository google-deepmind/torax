...
def extend_cell_grid_to_boundaries(
  ...
  """Extends a cell-centered array to the boundaries using the face-centered array."""
  axis = -1
  left = np.take(face_var, [0], axis=axis)  # pyrefly: ignore[no-matching-overload]
  right = np.take(face_var, [-1], axis=axis)  # pyrefly: ignore[no-matching-overload]
  return np.concatenate([left, cell_var, right], axis=axis)  # pyrefly: ignore[no-matching-overload]
