...
class PlotrunsLibTest(parameterized.TestCase):
  ...
  def test_get_limit_with_empty_attribute_data(self):
    profiles_ds = xr.Dataset({
        'T_i': (('time', 'rho'), np.empty((1, 0))),
    })
    top_level_ds = xr.Dataset({'time': [0.0]})
    data_tree = xr.DataTree(
        dataset=top_level_ds,
    ...
    with self.assertRaisesRegex(ValueError, 'No data supplied'):
      plotruns_lib._get_limit(
          plot_data, ('T_i',), 100.0, include_first_timepoint=False
      )
...
class FigurePropertiesTest(absltest.TestCase):
  ...
  def test_validation_raises_error(self):
    """Tests that ValueError is raised if axes exceed grid spots."""
...
class PlotDataTest(absltest.TestCase):
  ...
  def test_optional_attributes_default_to_zero(self):
    """Tests that missing optional attributes return a zero array."""
  ...
  def test_missing_attribute_raises_error(self):
    """Test that accessing a non-existent variable raises AttributeError."""
