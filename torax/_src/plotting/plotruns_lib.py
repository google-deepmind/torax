...
def load_data(filename: str) -> PlotData:
  ...
  def _transform_data(ds: xr.Dataset):
    ...
    transformations = {
        output.J_TOR_TOTAL: 1e6,  # A/m^2 to MA/m^2
        output.J_TOR_OHMIC: 1e6,  # A/m^2 to MA/m^2
        output.J_TOR_BOOTSTRAP: 1e6,  # A/m^2 to MA/m^2
        output.J_TOR_EXTERNAL: 1e6,  # A/m^2 to MA/m^2
  ...
  return PlotData(
      ...
      j_total=profiles_dataset[output.J_TOR_TOTAL].to_numpy(),
      j_ohmic=profiles_dataset[output.J_TOR_OHMIC].to_numpy(),
      j_bootstrap=profiles_dataset[output.J_TOR_BOOTSTRAP].to_numpy(),
      j_external=profiles_dataset[output.J_TOR_EXTERNAL].to_numpy(),
