...
class ImpurityFractionsTest(parameterized.TestCase):
  ...
  def test_impurity_fractions_output_fractions_mode(self):
    ...
    for i, t in enumerate(state_history.times):
      ...
      expected_w = impurity_config.species['C'].get_value(t)
      expected_fractions = np.array([expected_ne, expected_ar, expected_w])
  ...
  def test_impurity_fractions_output_ne_ratios_mode(self):
    ...
    for i, t in enumerate(state_history.times):
      ...
      expected_fractions = np.array([ne_ratio, ar_ratio, w_ratio]) / total_ratio
      np.testing.assert_allclose(
          fractions,
          expected_fractions,
          rtol=1e-5,
