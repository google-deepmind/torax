...
class WhileLoopBoundedTest(parameterized.TestCase):
  ...
  def test_output_history(self):
    ...
    # Steps after termination should be copies of the last state.
    for step in range(self._terminating_step + 1, self._max_steps + 1):
      chex.assert_trees_all_close(
          history_values[step],
          self._f_explicit(self._init_value, n_times=self._terminating_step),
      )


class WhileLoopUnboundedTest(parameterized.TestCase):
  ...
  def test_forward_agrees_with_while_loop(self):
    output_state, num_steps = jax_utils.while_loop_unbounded(
        self._cond_fun, self._body_fun, self.init_state
    )
    chex.assert_trees_all_close(
        output_state,
        jax.lax.while_loop(self._cond_fun, self._body_fun, self.init_state),
  ...
  def test_agrees_with_bounded(self):
    max_steps = 10
    bounded_state, bounded_steps, _ = jax_utils.while_loop_bounded(
        self._cond_fun, self._body_fun, self.init_state, max_steps
    )
    unbounded_state, unbounded_steps = jax_utils.while_loop_unbounded(
        self._cond_fun, self._body_fun, self.init_state
    )
    chex.assert_trees_all_close(bounded_state, unbounded_state)
    self.assertEqual(bounded_steps, unbounded_steps)
