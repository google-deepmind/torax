...
class NonlinearThetaMethod(solver.Solver):
  ...
  def _solve_step_helper(
    ...
    This helper method is called by `solve_step` after constructing the
    ...
    Args:
      prepared_state: Precomputed `dt`-independent solver state returned by
        `prepare_step`, containing `x_old`, `core_profiles_t`,
        `explicit_source_profiles`, `pedestal_transition_state`, `coeffs_old`,
        and `coeffs_exp_linear`.
      ...
      coeffs_callback: Calculates diffusion, convection etc. coefficients given
        core_profiles, geometry, and runtime_params. Repeatedly called by the
        iterative solvers.
