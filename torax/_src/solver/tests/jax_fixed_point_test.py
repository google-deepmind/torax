...
class FixedPointTest(parameterized.TestCase):
  ...
  def test_fixed_point_anderson(self):
    c1 = np.array([10, 12.0])
    c2 = np.array([3, 5.0])
    x = np.array([1.2, 1.3])
    ...
    out_expected = optimize.fixed_point(
        _func_np, x, args=(c1, c2), method='del2', maxiter=500, xtol=1e-8
    )
    ...
    chex.assert_trees_all_close(out_expected, out_jnp, atol=1e-5)

  def test_fixed_point_anderson_backtracking(self):
    c1 = np.array([10, 12.0])
    c2 = np.array([3, 5.0])
    x = np.array([1.2, 1.3])
