...
class BaseSolver(torax_pydantic.BaseModelFrozen, abc.ABC):
  ...
  fixed_point_method: Annotated[
      Literal['del2', 'iteration', 'anderson'], torax_pydantic.JAX_STATIC
  ] = 'iteration'
  anderson_window_size: Annotated[
      pydantic.NonNegativeInt, torax_pydantic.JAX_STATIC
  ] = 5
  anderson_safeguard_eta: float = 1.0
...
class LinearThetaMethod(BaseSolver):
  ...
  def build_runtime_params(self) -> runtime_params.RuntimeParams:
    return runtime_params.RuntimeParams(
        ...
        fixed_point_method=self.fixed_point_method,
        anderson_window_size=self.anderson_window_size,
        anderson_safeguard_eta=self.anderson_safeguard_eta,
