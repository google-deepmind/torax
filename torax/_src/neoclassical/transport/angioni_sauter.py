...
def _calculate_angioni_sauter_transport(
  ...
  epsilon = (geometry.R_out_face - geometry.R_in_face) / (
      geometry.R_out_face + geometry.R_in_face
  )

  B2_avg = geometry.B_0**2 / jnp.sqrt(1.0 - epsilon**2)
  Bm2_avg = geometry.B_0**-2 * (1.0 + 1.5 * epsilon**2)
  ...
  log_lambda_ii = collisions.calculate_log_lambda_ii(  # pylint: disable=protected-access
...
def _calculate_Kmn(
  ...
  Kmn_e = Kmn_e.at[:, 0, 2].set(-sauter.calculate_L31(ftrap, nu_e_star, Z_eff))
  Kmn_e = Kmn_e.at[:, 2, 0].set(Kmn_e[:, 0, 2])
  Kmn_e = Kmn_e.at[:, 1, 2].set(-sauter.calculate_L32(ftrap, nu_e_star, Z_eff))
