...
def _calculate_bootstrap_current(
  ...
  L31 = _calculate_L31(ftrap, nu_e_star, Z_eff_face)
  L32 = _calculate_L32(ftrap, nu_e_star, Z_eff_face)
  L34 = _calculate_L34(ftrap, nu_e_star, Z_eff_face)
  alpha = _calculate_alpha(ftrap, nu_i_star)
...
def _calculate_L31(
  ...
  """Calculates the L31 coefficient for the Sauter bootstrap model."""
  denom = (
      1.0
      + (1 - 0.1 * ftrap) * jnp.sqrt(nu_e_star)
      + 0.5 * (1.0 - ftrap) * nu_e_star / Z_eff
  )
  ft31 = ftrap / denom
...
def _calculate_L32(
  ...
  """Calculates the L32 coefficient for the Sauter bootstrap model."""
  # pylint: disable=invalid-name
  ft32ee = ftrap / (
      1
      + 0.26 * (1 - ftrap) * jnp.sqrt(nu_e_star)
      + 0.18 * (1 - 0.37 * ftrap) * nu_e_star / jnp.sqrt(Z_eff)
  )
  ft32ei = ftrap / (
      1
      + (1 + 0.6 * ftrap) * jnp.sqrt(nu_e_star)
      + 0.85 * (1 - 0.37 * ftrap) * nu_e_star * (1 + Z_eff)
  )
...
def _calculate_L34(
  ...
  """Calculates the L34 coefficient for the Sauter bootstrap model."""
  ft34 = ftrap / (
      1.0
      + (1 - 0.1 * ftrap) * jnp.sqrt(nu_e_star)
      + 0.5 * (1.0 - 0.5 * ftrap) * nu_e_star / Z_eff
  )
  return (
      (1 + 1.4 / (Z_eff + 1)) * ft34
      - 1.9 / (Z_eff + 1) * ft34**2
...
def _calculate_alpha(
  ...
  """Calculates the alpha coefficient for the Sauter bootstrap model."""
