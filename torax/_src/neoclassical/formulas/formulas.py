...
def calculate_bounce_averaged_trapped_fraction(
  ...
  """
  B_max = B.max()
  # Lambda is the normalized magnetic moment (aka orbit pitch parameter)
  lambda_ = jnp.linspace(0.0, 1.0, _LAMBDA_GRID_RESOLUTION) / B_max
  sqrt_term = jnp.sqrt(
      jnp.clip(1.0 - lambda_[:, jnp.newaxis] * B[jnp.newaxis, :], 0.0, None)
  )
  h_lambda = jnp.sum(sqrt_term * dl_over_Bp[jnp.newaxis, :], axis=1) / jnp.sum(
      dl_over_Bp
  )
  bounce_integral = jnp.trapezoid(
      lambda_, jnp.maximum(h_lambda, 1e-10), lambda_
  )
