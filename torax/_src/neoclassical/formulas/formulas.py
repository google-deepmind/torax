...
# Default number of grid points for the bounce-averaged lambda integration.
DEFAULT_LAMBDA_GRID_RESOLUTION: int = 101


def calculate_bounce_averaged_trapped_fraction(
    B: array_typing.Array,
    dl_over_Bp: array_typing.Array,
    flux_surf_avg_B2: array_typing.Array | float,
    n_lambda: int = DEFAULT_LAMBDA_GRID_RESOLUTION,
) -> array_typing.Array:
  ...
  Args:
    ...
    flux_surf_avg_B2: Flux surface average of :math:`B^2` for this flux surface
      [:math:`\mathrm{T}^2`].
    n_lambda: Number of grid points for the lambda integration.

  Returns:
    The effective trapped particle fraction of this flux surface.
  """
  B_max = B.max()
  lam = jnp.linspace(0.0, 1.0, n_lambda) / B_max
  sqrt_term = jnp.sqrt(
      jnp.clip(1.0 - lam[:, jnp.newaxis] * B[jnp.newaxis, :], 0.0, None)
  )
  h_lambda = jnp.sum(sqrt_term * dl_over_Bp[jnp.newaxis, :], axis=1) / jnp.sum(
      dl_over_Bp
  )
  bounce_integral = jnp.trapezoid(lam / jnp.maximum(h_lambda, 1e-10), lam)
