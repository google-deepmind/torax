...
"""Routines for calculating impurity charge states valid at the tokamak edge."""
...
_MAVRIN_2017_Z_COEFFS: Final[Mapping[str, array_typing.Array]] = (
...
_TEMPERATURE_INTERVALS_Z: Final[Mapping[str, array_typing.Array]] = (
    immutabledict.immutabledict({
...
def calculate_average_charge_single_species(
  ...
  """
  if ion_symbol not in _MAVRIN_2017_Z_COEFFS.keys():
    raise ValueError(
        f'Invalid ion symbol: {ion_symbol}. Allowed symbols are:'
        f' {_MAVRIN_2017_Z_COEFFS.keys()}'
    )
  ...
  interval_indices = jnp.searchsorted(
      _TEMPERATURE_INTERVALS_Z[ion_symbol], T_e_ev
  )
  Zavg_coeffs_in_range = jnp.take(
      _MAVRIN_2017_Z_COEFFS[ion_symbol], interval_indices, axis=1
  )
  ...
  log10_Z = (
      Zavg_coeffs_in_range[0]
      + Zavg_coeffs_in_range[1] * X
      + Zavg_coeffs_in_range[2] * Y
      + Zavg_coeffs_in_range[3] * X**2
      + Zavg_coeffs_in_range[4] * X * Y
      + Zavg_coeffs_in_range[5] * Y**2
      + Zavg_coeffs_in_range[6] * X**3
      + Zavg_coeffs_in_range[7] * X**2 * Y
      + Zavg_coeffs_in_range[8] * X * Y**2
      + Zavg_coeffs_in_range[9] * Y**3
  )
