...
class PsiCalculationsTest(parameterized.TestCase):
  ...
  def test_calc_bpol2(self):
    ...
    expected_Bpol2 = np.concatenate([np.array([0.0]), Bpol2_bulk])

    calculated_Bpol2 = psi_calculations.calc_bpol2(geo, psi_cell_variable)

    # Relatively low tolerance because the analytical formula is not exact for
    # our circular geometry, but approximates it at low inverse aspect ratio.
    np.testing.assert_allclose(calculated_Bpol2, expected_Bpol2, rtol=1e-3)
