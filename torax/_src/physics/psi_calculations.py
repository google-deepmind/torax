...
def _extrapolate_cell_profile_to_axis(
  ...
  i = jnp.argmax(mask)
  # cell_profile is a JitTracer, so we can index it with another JitTracer (i).
  clamp_profile_value = cell_profile[i]
  # geo.rho_norm is a jnp.array, so we need to explicitly dynamically index it.
  clamp_rho_norm_value = jax.lax.dynamic_index_in_dim(geo.rho_norm, i, axis=-1)
  return jnp.where(
    geo.rho_norm < clamp_rho_norm_value,
    clamp_profile_value,
    cell_profile,
  )
...
def calc_j_total(
  ...
  # Note: these will be nan on axis due to division by zero, but this is
  # handled by extrapolation below.
  j_total = dI_drhon / geo.spr
  j_total_face = dI_drhon_face / geo.spr_face

  # Extrapolate the axis values to avoid numerical artifacts
  j_total = _extrapolate_cell_profile_to_axis(j_total, geo)
  j_total_face = _extrapolate_face_profile_to_axis(j_total_face, j_total, geo)
