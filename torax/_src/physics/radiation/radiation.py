...
def calculate_cooling_rate(
  ...
  - For T_e > T_min_coronal, we use the coronal model.
  - For T_min_noncoronal < T_e < T_min_coronal, we use the noncoronal model.
  ...
  if ion_symbol_lookup not in mavrin_noncoronal_cooling_rate.COEFFS.keys():
    # For ions that do not have a noncoronal model, we set the cooling rate to 0
    noncoronal_cooling_rate = jnp.zeros_like(T_e)
  else:
  ...
  # Select the appropriate model based on the temperature
  # Smooth transition between noncoronal and coronal models over the range
  # T_e_min_coronal/2 to T_e_min_coronal*2.
  w = jax.nn.sigmoid((4 / jnp.log(2)) * jnp.log(T_e / T_e_min_coronal))
  return w * coronal_cooling_rate + (1 - w) * noncoronal_cooling_rate
