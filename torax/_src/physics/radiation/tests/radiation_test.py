...
class RadiationTest(parameterized.TestCase):
  ...
  def test_cooling_rate_zero_for_ions_without_noncoronal_model_in_noncoronal_regime(
      self,
  ):
    # W has no noncoronal model.
    # For T_e << T_e_min_coronal['W'], cooling rate should be ~0
    ion_symbol = 'W'
    T_e = np.array([0.001])
    ne_tau = 1e17
    ...
    self.assertLess(cooling_rate[0], 1e-40)
