...
def update_core_and_source_profiles_after_step(
  ...
  if (
      not dynamic_runtime_params_slice_t_plus_dt.numerics.evolve_current
      and dynamic_runtime_params_slice_t_plus_dt.profile_conditions.psidot
      is not None
  ):
    # If psidot is prescribed and current does not evolve, use prescribed value
    psidot_value = (
        dynamic_runtime_params_slice_t_plus_dt.profile_conditions.psidot
    )
  else:
