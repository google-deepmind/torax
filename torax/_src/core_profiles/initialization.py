...
def _init_psi_and_psi_derived(
  ...
  # Case 3: calculating j according to nu formula and psi from j.
    ...
    if (
      ...
      # In this branch we do not need external sources for the psi calculation.
      ...
      # This case requires non-inductive currents to determine j_total.
      # But non-inductive currents need a non-zero psi for the calculation.
      # We iterate once to get psi with Ohmic current only, and then refine.

      # 1. Initial psi guess: assume only ohmic current
      j_total_hires_ohmic = _get_j_total_hires_with_no_external_sources(
          dynamic_runtime_params_slice, geo
      )
      psi_ohmic = update_psi_from_j(
          dynamic_runtime_params_slice.profile_conditions.Ip,
          geo,
          j_total_hires_ohmic,
          use_v_loop_lcfs_boundary_condition=dynamic_runtime_params_slice.profile_conditions.use_v_loop_lcfs_boundary_condition,
      )
      core_profiles_ohmic = dataclasses.replace(
          core_profiles,
          psi=psi_ohmic,
          q_face=psi_calculations.calc_q_face(geo, psi_ohmic),
          s_face=psi_calculations.calc_s_face(geo, psi_ohmic),
      )

      # TODO(b/440385263): add tunable iteration number or convergence criteria.
      # 2. Iterate with non-inductive current source calculations. Stop after 2.

      psi, source_profiles = _iterate_psi_and_sources(
          dynamic_runtime_params_slice=dynamic_runtime_params_slice,
          geo=geo,
          core_profiles=core_profiles_ohmic,
  ...
  # --- Post-psi calculation steps ---
  # At this point, `psi` is determined for all cases.
