...
def _calculate_all_psi_dependent_profiles(
  ...
  if (
      not dynamic_runtime_params_slice.numerics.evolve_current
      and dynamic_runtime_params_slice.profile_conditions.psidot is not None
  ):
    # If psidot is prescribed and psi does not evolve, use prescribed value
    psidot_value = dynamic_runtime_params_slice.profile_conditions.psidot
  else:
