...
class PlasmaCompositionTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          expected_Z_override=6.5,
          expected_A_override=13.0,
          expected_impurity_model_type=n_e_ratios.NeRatiosModel,
      ),
      dict(
  ...
  def test_ne_ratios_avg_a_calculation(self):
    ...
    assert isinstance(
        dynamic_impurity_ne_ratios, n_e_ratios.DynamicNeRatios
    )
  ...
  def test_ne_ratios_model_under_jit(self):
    ...
    output = f(pc, 0.0)
    self.assertIsInstance(output.impurity, n_e_ratios.DynamicNeRatios)
    self.assertEqual(jax_utils.get_number_of_compiles(f), 1)
