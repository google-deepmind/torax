...
class InitializationTest(parameterized.TestCase):
  ...
  def test_get_initial_psi_source_geometry(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'geometry',
    }
    torax_config = model_config.ToraxConfig.from_dict(config)
    ...
    psi_source = initialization._get_initial_psi_source(
        runtime_params, mock.ANY,
    )
    self.assertEqual(
        psi_source, profile_conditions_lib.InitialPsiSource.GEOMETRY)

  def test_get_initial_psi_source_j(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'j',
    }
    torax_config = model_config.ToraxConfig.from_dict(config)
    params_provider = build_runtime_params.RuntimeParamsProvider.from_config(
        torax_config
    ...
    self.assertEqual(
        psi_source, profile_conditions_lib.InitialPsiSource.J)

  def test_get_initial_psi_source_profile_conditions(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'profile_conditions',
        'psi': 15.,
    }
    torax_config = model_config.ToraxConfig.from_dict(config)
    params_provider = build_runtime_params.RuntimeParamsProvider.from_config(
    ...
    self.assertEqual(
        psi_source, profile_conditions_lib.InitialPsiSource.PROFILE_CONDITIONS)

  def test_get_initial_psi_source_legacy_initial_psi_from_j(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'profile_conditions',
        'initial_psi_from_j': True,
    ...
    runtime_params = params_provider(t=0.0)
    psi_source = initialization._get_initial_psi_source(
        runtime_params, torax_config.geometry.build_provider(t=0.0),
    )
    self.assertEqual(
        psi_source, profile_conditions_lib.InitialPsiSource.J)

  def test_get_initial_psi_source_legacy_initial_psi_from_j_circular_geo(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'profile_conditions',
        'initial_psi_from_j': False,
  ...
  def test_get_initial_psi_source_legacy_init_from_geo(self):
    config = default_configs.get_default_config_dict()
    config['profile_conditions'] = {
        'initial_psi_source': 'profile_conditions',
        'initial_psi_from_j': False,
