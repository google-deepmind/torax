...
class InitializationTest(parameterized.TestCase):
  ...
  def test_initial_core_profiles_velocities(self):
    ...
    np.testing.assert_allclose(
        core_profiles.toroidal_velocity, np.zeros_like(geo.rho)
    )
    np.testing.assert_allclose(
        core_profiles.poloidal_velocity, np.zeros_like(geo.rho)
    )
  ...
  def test_initial_toroidal_velocity_from_profile_conditions(self):
    ...
    v_phi_test = np.array([10.0, 20.0, 30.0, 40.0])
    _, geo, _ = _get_initial_state(
        model_config.ToraxConfig.from_dict(config)
    )
    config['profile_conditions']['toroidal_velocity'] = {
        0.0: {rho: value for rho, value in zip(geo.rho_norm, v_phi_test)}
    }
    torax_config = model_config.ToraxConfig.from_dict(config)
    core_profiles, _, _ = _get_initial_state(torax_config)
    np.testing.assert_allclose(core_profiles.toroidal_velocity, v_phi_test)
    np.testing.assert_allclose(
        core_profiles.poloidal_velocity, np.zeros_like(geo.rho)
    )
