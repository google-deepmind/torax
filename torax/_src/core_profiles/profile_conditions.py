...
class InitialPsiSource(enum.StrEnum):
  """Source of the initial psi value."""
...
class RuntimeParams:
  ...
  initial_psi_source: InitialPsiSource = dataclasses.field(
      metadata={'static': True}
  )
...
class ProfileConditions(torax_pydantic.BaseModelFrozen):
  ...
  Attributes:
    ...
    initial_psi_source: Source of the initial psi value. If `PROFILE_CONDITIONS`
  ...
  initial_psi_source: Annotated[InitialPsiSource, torax_pydantic.JAX_STATIC] = (
      InitialPsiSource.PROFILE_CONDITIONS
  )
