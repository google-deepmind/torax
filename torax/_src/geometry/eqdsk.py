...
def _construct_intermediates_from_eqdsk(
  ...
  exact_trapped_fraction = (
      ...
      if trapped_fraction_source == base.TrappedFractionSource.EXACT
      else None
  ...
  for n, (x_surface, z_surface) in enumerate(surfaces):
    ...
    if exact_trapped_fraction is not None:
      surface_B = np.sqrt(surface_B2)
      exact_trapped_fraction[n + 1] = (
  ...
  match trapped_fraction_source:
    case base.TrappedFractionSource.EXACT:
      ...
      assert exact_trapped_fraction is not None
      exact_is_unreliable = (
      ...
      if np.any(exact_is_unreliable):
        logging.warning(
            'Overwriting %d bad/unphysical EXACT trapped fraction values with'
            ' Sauter approximation.',
