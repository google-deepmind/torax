...
class StandardGeometryIntermediates:
    ...
    def __post_init__(self):
        ...
        if self.flux_surf_avg_Bp2[-1] < 1e-10:
            ...
            def spline(rho, data, x, bc_type): return scipy.interpolate.CubicSpline(
                rho[:-1],
                data[:-1],
                bc_type=bc_type,
            )(x)
            ...
            def set_edge(array): return spline(rhon, array, 1.0, bc_type)
    ...
    def from_chease(
        ...
        Ip_chease = chease_data["Ipprofile"] / \
            constants.CONSTANTS.mu_0 * R_major * B_0
        ...
        flux_surf_avg_RBp = chease_data["<|grad(psi)|>"] * \
            psiunnormfactor / R_major
        flux_surf_avg_R2Bp2 = (
    ...
    def _get_LY_single_slice_from_bundle(
        ...
        LY_single_slice = {key: LY_bundle[key]
                           [..., idx] for key in relevant_keys}
    ...
    def from_eqdsk(
        ...
        # Initialise arrays
        areas, volumes = np.empty(
            len(surfaces) + 1), np.empty(len(surfaces) + 1)
        R_inboard, R_outboard = np.empty(
            len(surfaces) + 1), np.empty(len(surfaces) + 1)
        ...
        flux_surf_avg_R2Bp2_eqdsk = np.empty(
            len(surfaces) + 1)  # <|grad(psi)|**2>
        ...
        int_dl_over_Bp_eqdsk = np.empty(
            len(surfaces) + 1)  # int(Rdl / | grad(psi) |)
        Ip_eqdsk = np.empty(len(surfaces) + 1)  # Toroidal plasma current
        delta_upper_face_eqdsk = np.empty(
            len(surfaces) + 1)  # Upper face delta
        delta_lower_face_eqdsk = np.empty(
            len(surfaces) + 1)  # Lower face delta
        ...
        for n, (x_surface, z_surface) in enumerate(surfaces):
            ...
            surface_dpsi_x = psi_2dgrid_interpolator.ev(
                x_surface, z_surface, dx=1)
            surface_dpsi_z = psi_2dgrid_interpolator.ev(
                x_surface, z_surface, dy=1)
            surface_abs_grad_psi = np.sqrt(
                surface_dpsi_x**2 + surface_dpsi_z**2)
            ...
            surface_delta_upper_face = (
                R_major_local - X_upperextent) / a_minor_local
            surface_delta_lower_face = (
                R_major_local - X_lowerextent) / a_minor_local
            ...
            elongation[n + 1] = (Z_upperextent -
                                 Z_lowerextent) / (2.0 * a_minor_local)
        ...
        vpr = 4 * np.pi * Phi_eqdsk[-1] * rhon / \
            (F * flux_surf_avg_1_over_R2_eqdsk)
...
def build_standard_geometry(
    ...
    def rhon_interpolation_func(x, y): return np.interp(
    ...
    g2g3_over_rhon_face = rhon_interpolation_func(
        rho_face_norm, g2g3_over_rhon)
    ...
    gm4 = rhon_interpolation_func(
        rho_norm, intermediate.flux_surf_avg_1_over_B2)
...
def _validate_fbt_data(
    ...
    if missing_LY_keys:
        raise ValueError(f"LY data is missing the following keys: {
                         missing_LY_keys}")
