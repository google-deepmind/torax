...
def face_to_cell(face: array_typing.Array) -> array_typing.Array:
...
class Geometry:
  ...
  R_major: array_typing.Array
  a_minor: array_typing.Array
  B_0: array_typing.Array
  ...
  Phi_b_dot: array_typing.Array
  _z_magnetic_axis: array_typing.Array | None
...
def stack_geometries(geometries: Sequence[Geometry]) -> Geometry:
  ...
  for field in dataclasses.fields(first_geo):
    ...
    if isinstance(field_value, array_typing.Array) or isinstance(
        field_value, float
    ):
