...
class TGLFTransportModel(
  ...
  def __init__(self):
    ...
    self._tglfrun_name = (
        _DEFAULT_TGLFRUN_NAME_PREFIX + '_' + timestamp + '_' + unique_suffix
    )
  ...
  def _run_tglf(
    ...
    # Spawns a new "server" process, from which child processes are forked to
    # farm out single-radius TGLF runs in parallel.
    # Safer than using fork method, and faster than using spawn method.
    ...
    with ctx.Manager() as manager:
      ...
      if verbose:
        # Use to print detailed stdout / stderr messages from child processes
...
def _extract_tglf_plan(
  ...
  Returns:
      A list of dictionaries containing TGLF input namelists
  """
  ...
  for species_number in range(1, 4):
    tglf_input_template.update({f'{key}_{species_number}': value for key, value in species_template.items()})
  ...
  for i, rho in enumerate(np.array(geo.rho_face_norm)):
    ...
    tglf_runpars['MASS_1'] = float(constants.CONSTANTS.m_e / (constants.CONSTANTS.m_amu * constants.ION_PROPERTIES_DICT['D'].A))
...
def _run_tglf_single(run, queue):
  """ Function to insert TGLF run command into multiprocessing queue for parallelization. """

  # Each TGLF run hardcoded to run with 2 cores, benefit from having more simultaneous runs
  # seen to outweigh higher parallelization of single run.
  command = [
      run['execution_path'],
      '-n',
      str(2),
...
class TGLFTransportModelConfig(pydantic_model_base.TransportBase):
  ...
  Attributes:
    ...
    n_processes: Set number of parallel TGLF calculations to run, each using 2 cores.
    ...
    kygrid_model: 0 = user-defined with n_ky points equally spaced with kymin = ky/n_ky.
      1 = standard ky spectrum for SAT0 and SAT1 with kymin=0.1/rho_i.
      4 = standard ky spectrum with more low ky points and kymin=0.05*grad_r0/rho_i.
    ky: Specify wavenumber for single wavenumber call, or set user-defined ky grid with kygrid_model=0.
    n_ky: Number of ky points with kygrid_model=0, else number of additional logarithmically
      equally spaced points within 1 < ky < 24 when using kygrid_model=4.
    n_modes: Number of eigenmodes to track, advise to use num_species+2 for efficiency.
    geometry_flag: 0 = s-alpha, 1 = Miller/MXH, 2 = Fourier, 3 = ELITE.
    sat_rule: Specify quasilinear saturation rule used to compute transport fluxes.
    xnu_model: Specify collision model. 2 = default for SAT0 and SAT1, 3 = default for SAT2 and SAT3.
    n_width: Maximum number of mode widths in mode width scan.
    width_min: Minimum value for mode width scan, set negative for electromagnetic search.
    width: Maximum value for mode width scan.
    filter: Set frequency threshold to filter out non-drift-wave instabilities.
    theta_trapped: Adjustment parameter for trapped fraction model. Set to 0.4 with n_basis_max = 8
      for low aspect ratio (https://eprints.whiterose.ac.uk/159700/).
    ...
    xnu_factor: Multiplier for the trapped/passing boundary collision terms, not the same as collisionality_multiplier.
    debye_factor: Multiplier for the normalized Debye length.
    etg_factor: Exponent for the ETG saturation rule.
    find_width: Toggle mode width scan for maximum growth rate search, uses width argument if False.
    use_mhd_rule: If True, ignore pressure gradient contribution to curvature drift. Recommended to set False for high beta.
    use_bpar: If True, include compressional magnetic fluctuations, :math:`\delta B_{\par}`.
    use_bper: If true, include transverse magnetic fluctuations, :math:`\delta B_{\perp}`.
    use_inboard_detrapped: If True, set trapped fraction to zero if eigenmode is inward ballooning.
    use_ave_ion_grid: If True, use average ion gyroradius as opposed to main ion gyroradius
    alpha_e: Multiplier for ExB velocity shear for spectral shift model.
    alpha_zf: Zonal flow mixing coefficient. If -1.0, toggles switch that avoids picking lowest ky as maximum gamma/ky
      for intensity spectrum shape in quasilinear saturation rules.
    alpha_quench: 0.0 = use spectral shift model, 1.0 = use quench rule.
    n_xgrid: Number of nodes in Gauss-Hermite quadrature. Recommended to use 4 * n_basis_max
    n_basis_min: Minimum number of parallel basis functions (Hermite polynomials) used to find mode width.
    n_basis_max: Maximum number of parallel basis functions (Hermite polynomials) used to find mode width.
      Recommended 4 for SAT0 and  SAT1, 6 for SAT2 and SAT3.
  """
  ...
  # Species settings
  #n_species: pydantic.PositiveInt = 3 # currently hardcoded as 3 in TORAX
