...
_DEFAULT_TGLF_NAME_PREFIX = 'torax_tglf_runs'
...
class TGLFTransportModel(
  ...
  def __init__(self):
    self._tglf_parentdir = tempfile.TemporaryDirectory()
    self._tglf_name = (
        _DEFAULT_TGLF_NAME_PREFIX
        + datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    )
    self._runpath = os.path.join(self._tglf_parentdir.name, self._tglf_name)
    self._frozen = True
  ...
  def _call_implementation(
    ...
    self._run_tglf(
        transport_dynamic_runtime_params.n_processes
    )
  ...
  def _run_tglf(
    ...
    process = subprocess.Popen(
        ...
        cwd=f'{self._tglf_parentdir.name}/{self._tglf_name}',
    )
...
class TGLFTransportModelConfig(pydantic_model_base.TransportBase):
  ...
  model_name: Literal['tglf'] = 'tglf'
  n_processes: pydantic.PositiveInt = 8
  sat_rule: pydantic.PositiveInt = 2
  collision_model: pydantic.PositiveInt = 3
