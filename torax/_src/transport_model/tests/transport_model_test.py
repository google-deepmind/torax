...
class FakeTransportConfig(transport_pydantic_model_base.TransportBase):
    ...
    model_name: Annotated[Literal["fake_transport_test"], torax_pydantic.JAX_STATIC] = (
        "fake_transport_test"
    )
...
class TransportSmoothingTest(parameterized.TestCase):
    ...
    def setUp(self):
        super().setUp()
        # Save original annotation for restoration in tearDown.
    ...
    def tearDown(self):
        super().tearDown()
        # Restore original annotation to avoid polluting other tests.
        model_config.ToraxConfig.model_fields[
            "transport"
    ...
    def test_smoothing(self):
        ...
        config["transport"] = {
            "model_name": "fake_transport_test",
        ...
        smoothing_array = np.where(smoothing_array < lower_cutoff, 0.0, smoothing_array)
        smoothing_array /= np.sum(smoothing_array)
...
class TransportMaskingTest(parameterized.TestCase):
    ...
    def test_single_model_masking(self):
        ...
        transport_model = torax_config.transport.build_transport_model()
        coeffs = transport_model(runtime_params, geo, core_profiles, pedestal_outputs)
    ...
    def test_combined_model_masking(self):
        ...
        ref_coeffs = single_model(single_runtime, geo, core_profiles, pedestal_outputs)
    ...
    def test_preserves_none_enabled(self):
        ...
        new_coeffs = model.zero_out_disabled_channels(runtime_params.transport, coeffs)
    ...
    def test_preserves_none_disabled(self):
        ...
        new_coeffs_disabled = model.zero_out_disabled_channels(disabled_params, coeffs)
