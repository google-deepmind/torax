...
class NormalizedLogarithmicGradients:
  ...
  fast_ion_gradients: Mapping[
      str, tuple[array_typing.FloatVectorFace, array_typing.FloatVectorFace]
  ]
  ...
  def from_profiles(
    ...
    for fi in core_profiles.fast_ions:
      ...
      fi_grads[key] = (lref_over_ln, lref_over_lt)
...
def calculate_alpha(
  ...
  for fi in core_profiles.fast_ions:
    key = f"{fi.source}_{fi.species}"
    lref_over_ln, lref_over_lT = (
        normalized_logarithmic_gradients.fast_ion_gradients[key]
    )
    alpha += factor_0 * (
