  ...
  def _prepare_qualikiz_inputs(
    ...
    mach_toroidal = (
        ...
        * geo.R_major_profile_face
        / c_ref
    ) * transport.rotation_multiplier

    if transport.rotation_mode == RotationMode.OFF:
      zero_face = jnp.zeros_like(core_profiles.q_face)
      gamma_E_QLK = zero_face
      gamma_E_GB = zero_face
      gamma_E_GB_poloidal_and_pressure = zero_face
      gamma_E_GB_toroidal = zero_face
    else:
