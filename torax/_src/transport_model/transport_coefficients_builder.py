...
def calculate_all_transport_coeffs(
  ...
  if (
    ...
    is_L_mode = jnp.logical_not(pedestal_transition_state.in_H_mode)
    is_transitioning = jnp.logical_and(
    ...
    need_mask = jnp.logical_or(jnp.logical_not(is_L_mode), is_transitioning)
