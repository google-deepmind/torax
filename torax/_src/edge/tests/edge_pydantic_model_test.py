...
class ExtendedLengyelPydanticModelTest(absltest.TestCase):
  ...
  def test_enrichment_factor_key_validation(self):
    with self.subTest('valid_enrichment_factor'):
    ...
    with self.subTest('missing_key_in_enrichment_factor'):
    ...
    with self.subTest('extra_key_in_enrichment_factor'):
      with self.assertRaisesRegex(
          ValueError,
  ...
  def test_computation_mode_inputs_validation(self):
    with self.subTest('valid_forward'):
      pydantic_model.ExtendedLengyelConfig(
          computation_mode='forward',
          target_electron_temp=None,
          seed_impurity_weights={},
      )
    with self.subTest('invalid_forward_with_target_temp'):
      with self.assertRaisesRegex(
          ValueError,
    ...
    with self.subTest('invalid_forward_with_seed_impurities'):
      with self.assertRaisesRegex(
          ValueError,
    ...
    with self.subTest('valid_inverse'):
      pydantic_model.ExtendedLengyelConfig(
          computation_mode='inverse',
          target_electron_temp=10.0,
          seed_impurity_weights={'N': 1.0},
    ...
    with self.subTest('invalid_inverse_missing_target_temp'):
      with self.assertRaisesRegex(
          ValueError,
    ...
    with self.subTest('invalid_inverse_missing_seed_impurities'):
      with self.assertRaisesRegex(
          ValueError,
