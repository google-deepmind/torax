...
class ExtendedLengyelFormulasTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    self.separatrix_electron_temp = 100.0  # eV
    self.average_ion_mass = 2.0  # amu
    self.magnetic_field_on_axis = 2.5
    self.plasma_current = 1e6
    self.major_radius = 1.65
    self.minor_radius = 0.5
    self.elongation_psi95 = 1.6
    self.triangularity_psi95 = 0.3
    self.power_crossing_separatrix = 5.5e6
  ...
  def test_calc_momentum_loss_in_convection_layer(self):
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-6,
        err_msg=(
            'calc_momentum_loss_in_convection_layer does not match the'
            ' reference value.'
        ),
    )

  def test_calc_density_loss_in_convection_layer(self):
    # reference value from the first loop of the reference case in
    # https://github.com/cfs-energy/extended-lengyel
    expected_density_loss = 0.6108818435013572
    calculated_density_loss = (
        extended_lengyel_formulas.calc_density_loss_in_convection_layer(
            target_electron_temp=self.target_electron_temp
        )
    )
    np.testing.assert_allclose(
        calculated_density_loss,
        expected_density_loss,
        rtol=1e-6,
        err_msg=(
            'calc_density_loss_in_convection_layer does not match the'
            ' reference value.'
        ),
    )
  ...
  def test_calc_power_loss_in_convection_layer(self):
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-6,
        err_msg=(
            'calc_power_loss_in_convection_layer does not match the'
            ' reference value.'
        ),
    )
  ...
  def test_calc_shaping_factor(self):
    ...
    np.testing.assert_allclose(
        ...
        expected_value,
        rtol=1e-5,
        err_msg='calc_shaping_factor does not match the reference value.',
    )
  ...
  def test_calc_separatrix_average_poloidal_field(self):
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-5,
        err_msg=(
            'calc_separatrix_average_poloidal_field does not match the'
            ' reference value.'
        ),
    )
  ...
  def test_calc_cylindrical_safety_factor(self):
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-5,
        err_msg=(
            'calc_cylindrical_safety_factor does not match the reference value.'
        ),
    )
  ...
  def test_calc_fieldline_pitch_at_omp(self):
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-5,
        err_msg=(
            'calc_fieldline_pitch_at_omp does not match the reference value.'
        ),
    )
  ...
  def test_calculate_q_parallel(self):
    ...
    calculated_q_parallel = extended_lengyel_formulas.calculate_q_parallel(
        separatrix_electron_temp=self.separatrix_electron_temp,
        average_ion_mass=self.average_ion_mass,
        separatrix_average_poloidal_field=separatrix_average_poloidal_field,
        alpha_t=alpha_t,
        ratio_of_upstream_to_average_poloidal_field=extended_lengyel_defaults.RATIO_UPSTREAM_TO_AVG_BPOL,
        ...
        major_radius=self.major_radius,
        minor_radius=self.minor_radius,
        power_crossing_separatrix=self.power_crossing_separatrix,
    ...
    np.testing.assert_allclose(
        ...
        expected_q_parallel,
        rtol=1e-5,
        err_msg='q_parallel calculation does not match the reference value.',
    )
  ...
  def test_calc_Z_eff(self):
    ...
    np.testing.assert_allclose(
        ...
        expected_Z_eff,
        rtol=1e-5,
        err_msg='Z_eff calculation does not match the expected value.',
    )
