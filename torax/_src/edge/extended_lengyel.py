...
class ExtendedLengyelOutputs:
  ...
  separatrix_z_effective: array_typing.FloatScalar
...
def run_extended_lengyel_model(
  ...
  return ExtendedLengyelOutputs(
      neutral_pressure_in_divertor=0.0,
      alpha_t=alpha_t,
      q_parallel=0.0,
      heat_flux_perp_to_target=0.0,
      separatrix_electron_temp=separatrix_electron_temp,
      separatrix_z_effective=0.0,
...
def _solve_for_c_z(
  ...
  # Ensure physical result (power cannot be negative). A power of zero will be
  # handled as an error in the calling function.
  q_div_squared = jnp.maximum(q_div_squared, 0.0)
