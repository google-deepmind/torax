...
class SolveQccStatus(enum.IntEnum):
  """Status of the _solve_for_qcc calculation.
...
class ExtendedLengyelOutputs:
  ...
  heat_flux_perp_to_target: array_typing.FloatScalar
  separatrix_electron_temp: array_typing.FloatScalar
  separatrix_Z_eff: array_typing.FloatScalar
  impurity_concentrations: Mapping[str, array_typing.FloatScalar]
...
def run_extended_lengyel_model(
    ...
    loop_iterations: int = extended_lengyel_defaults.LOOP_ITERATIONS,
  ...
  Args:
    ...
    loop_iterations: Number of iterations for fixed point solver.
  ...
  kappa_e_init = extended_lengyel_formulas.KAPPA_E_0
...
def _inverse_mode_fixed_step_solver(
  ...
  for _ in range(loop_iterations):
    ...
    c_z_prefactor, _ = _solve_for_c_z_prefactor(sol_state=sol_state)
    sol_state = dataclasses.replace(sol_state, c_z_prefactor=c_z_prefactor)
    ...
    sol_state = dataclasses.replace(
        sol_state,
        kappa_e=divertor_sol_1d_lib.calc_kappa_e(sol_state.divertor_Z_eff),
        alpha_t=alpha_t,
    )
  return sol_state
...
def _forward_mode_fixed_step_solver(
  ...
  for i in range(loop_iterations):
    ...
    target_electron_temp = divertor_sol_1d_lib.calc_target_electron_temp(
        sol_state=sol_state,
        parallel_heat_flux_at_cc_interface=q_cc,
        previous_target_electron_temp=sol_state.target_electron_temp,
    )
    sol_state = dataclasses.replace(
        sol_state, target_electron_temp=target_electron_temp
    )

    # Update kappa_e and alpha_t for the next iteration.
    kappa_e = divertor_sol_1d_lib.calc_kappa_e(sol_state.divertor_Z_eff)
    sol_state = dataclasses.replace(sol_state, kappa_e=kappa_e)
    ...
    sol_state = dataclasses.replace(sol_state, alpha_t=alpha_t)
...
def _solve_for_c_z_prefactor(
    sol_state: divertor_sol_1d_lib.DivertorSOL1D,
) -> tuple[array_typing.FloatScalar, SolveCzStatus]:
...
def _solve_for_qcc(
    sol_state: divertor_sol_1d_lib.DivertorSOL1D,
) -> tuple[array_typing.FloatScalar, SolveQccStatus]:
  ...
  Returns:
      ...
      status: A SolveQccStatus enum indicating the outcome of the calculation.
  """
  # Temperatures must be in keV for the L_INT calculation.
  cc_temp_keV = sol_state.electron_temp_at_cc_interface / 1000.0
  div_temp_keV = sol_state.divertor_entrance_electron_temp / 1000.0
  ...
  status = jnp.where(
      qcc_squared < 0.0,
      SolveQccStatus.Q_CC_SQUARED_NEGATIVE,
      SolveQccStatus.SUCCESS,
  )

  qcc = jnp.where(status == SolveQccStatus.SUCCESS, jnp.sqrt(qcc_squared), 0.0)
