...
# Coefficients for temperature fit functions.
# From Table 1 in T. Body et al 2025 Nucl. Fusion 65 086002.
# To be used with the Stangeby function _temperature_fit_function.
_MOMENTUM_LOSS_AMPLITUDE = 0.8858679172531956
_MOMENTUM_LOSS_WIDTH = 3.8263045353064467
_MOMENTUM_LOSS_SHAPE = 0.8282347762381935

_DENSITY_LOSS_AMPLITUDE = 0.5587910467003282
_DENSITY_LOSS_WIDTH = 2.020427078509838
_DENSITY_LOSS_SHAPE = 0.9600157520406738

_POWER_LOSS_AMPLITUDE = 0.8532115334413933
_POWER_LOSS_WIDTH = 5.195481324376164
_POWER_LOSS_SHAPE = 0.9642427916765323

KAPPA_E_0: Final[array_typing.FloatScalar] = (
    2390  # Spitzer parallel conductivity prefactor [W/ m (m * ev**3.5)]
)
...
def _temperature_fit_function(
  ...
  Args:
    ...
    amplitude: Amplitude of the fit function.
    width: Width of the fit function.
    shape: Shape parameter of the fit function.

  Returns:
    The value of the fit function.
  """
  return 1.0 - amplitude * jnp.power(
      1.0 - jnp.exp(-target_electron_temp / width), shape
  )


def calc_momentum_loss_in_convection_layer(
    target_electron_temp: array_typing.FloatScalar,
    amplitude: float = _MOMENTUM_LOSS_AMPLITUDE,
    width: float = _MOMENTUM_LOSS_WIDTH,
    shape: float = _MOMENTUM_LOSS_SHAPE,
) -> array_typing.FloatScalar:
  """Calculates the momentum loss in the convection layer."""
  return _temperature_fit_function(
      target_electron_temp, amplitude, width, shape
  )


def calc_density_loss_in_convection_layer(
    target_electron_temp: array_typing.FloatScalar,
    amplitude: float = _DENSITY_LOSS_AMPLITUDE,
    width: float = _DENSITY_LOSS_WIDTH,
    shape: float = _DENSITY_LOSS_SHAPE,
) -> array_typing.FloatScalar:
...
def calc_power_loss_in_convection_layer(
    target_electron_temp: array_typing.FloatScalar,
    amplitude: float = _POWER_LOSS_AMPLITUDE,
    width: float = _POWER_LOSS_WIDTH,
    shape: float = _POWER_LOSS_SHAPE,
) -> array_typing.FloatScalar:
...
def calc_cylindrical_safety_factor(
  ...
  See equations 49-56 from T. Body et al 2025 Nucl. Fusion 65 086002,
...
def calc_electron_temp_at_cc_interface(
  ...
  density_loss = calc_density_loss_in_convection_layer(target_electron_temp)
...
def calculate_q_parallel(
  ...
  # Body NF 2025 Eq 49.
  separatrix_average_lambda_Te = (
      2.1 * (1.0 + 2.1 * alpha_t**1.7) * separatrix_average_rho_s_pol
  )
