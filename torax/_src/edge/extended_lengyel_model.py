...
@jax.tree_util.register_dataclass
@dataclasses.dataclass(frozen=True)
class ResolvedGeometricParams:
  """Container for geometric parameters after resolution."""
...
class ExtendedLengyelModel(base.EdgeModel):
  ...
  def __call__(
    ...
    # warnings.
    resolved_geo_params = self._resolve_geometric_parameters(
    ...
    power_crossing_separatrix = jnp.maximum(P_SOL_total, 1e-3)
    ...
    return extended_lengyel_standalone.run_extended_lengyel_standalone(
        ...
        ratio_of_upstream_to_average_poloidal_field=resolved_geo_params.ratio_of_upstream_to_average_poloidal_field,
  ...
  def _resolve_geometric_parameters(
    ...
    if geo.B_pol_OMP is not None:
      bpol_avg = jnp.sqrt(psi_calculations.calc_bpol2(geo, core_profiles.psi))
      geo_ratio_bpol = jnp.abs(geo.B_pol_OMP) / jnp.sqrt(bpol_avg)
    else:
      geo_ratio_bpol = None
    ...
    return ResolvedGeometricParams(
        ...
        ratio_of_upstream_to_average_poloidal_field=ratio_bpol,
