...
class ExtendedLengyelOutputs(base.EdgeModelOutputs):
  ...
  def to_output_dict(
    ...
    if self.calculated_enrichment:
      ...
      if self.impurity_right_bc and set(impurities) != set(
          self.impurity_right_bc.keys()
      ):
        sample_arr = next(iter(self.impurity_right_bc.values()))
        nan_array = np.full_like(np.asarray(sample_arr), np.nan)
        bc_data = np.stack(
            [
                np.asarray(self.impurity_right_bc.get(i, nan_array))
                for i in impurities
            ],
            axis=0,
        )
    ...
    divertor_quantities: dict[str, output_grid_context.OutputVar] = {
        str(Q_PARALLEL): context.pack(Q_PARALLEL, self.q_parallel),
        str(Q_PERPENDICULAR_TARGET): context.pack(
            Q_PERPENDICULAR_TARGET, self.q_perpendicular_target
        ),
        str(T_E_SEPARATRIX): context.pack(T_E_SEPARATRIX, self.T_e_separatrix),
        str(T_E_TARGET): context.pack(T_E_TARGET, self.T_e_target),
        str(PRESSURE_NEUTRAL_DIVERTOR): context.pack(
            PRESSURE_NEUTRAL_DIVERTOR,
  ...
  def to_xr_datatree(
    ...
    elif self.impurity_right_bc:
      coords[output_keys.IMPURITY] = sorted(
          list(self.impurity_right_bc.keys())
      )
