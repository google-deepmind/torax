...
"""Routines for calculating impurity cooling curves valid at the tokamak edge."""
...
DEFAULT_MODEL_FUNCTION_NAME: str = 'mavrin_fit_edge'
...
_MAVRIN_2017_L_COEFFS: Final[Mapping[str, array_typing.FloatVector]] = (
...
_TEMPERATURE_INTERVALS_L: Final[Mapping[str, array_typing.FloatVector]] = (
    immutabledict.immutabledict({
...
_MIN_TEMPERATURES: Final[Mapping[str, float]] = immutabledict.immutabledict({
    'He': 1.0,
    'Li': 1.0,
    'Be': 0.2,
    'C': 1.0,
...
_MAX_TEMPERATURES: Final[Mapping[str, float]] = immutabledict.immutabledict({
    'He': 1.5e4,
    'Li': 1.0e4,
    'Be': 1.0e4,
    'C': 1.5e4,
...
def calculate_radiative_cooling_rate_single_species(
    T_e: array_typing.FloatVector,
    ne_tau: array_typing.FloatScalar,
    ion_symbol: str,
) -> array_typing.FloatVector:
  """Calculates the radiation cooling rate valid at the edge, for a single impurity.

  Polynomial fit range is 1 eV-15 keV, which is well within the typical
  ...
  Args:
    ...
    ne_tau: The residence parameter, being the product of electron density and
      impurity residence time [m^-3 s]. High values of ne_tau correspond to
      the coronal limit.
  ...
  """
  if ion_symbol not in _MAVRIN_2017_L_COEFFS.keys():
    raise ValueError(
        f'Invalid ion symbol: {ion_symbol}. Allowed symbols are :'
        f' {_MAVRIN_2017_L_COEFFS.keys()}'
    )
  ...
  min_temp = _MIN_TEMPERATURES[ion_symbol]
  max_temp = _MAX_TEMPERATURES[ion_symbol]
  T_e_ev = jnp.clip(T_e_ev, min_temp, max_temp)
  # Residence parameter capped at 10^19, which is the coronal limit.
  ne_tau = jnp.clip(ne_tau, a_max=1e19)

  # Gather coefficients for each temperature
  interval_indices = jnp.searchsorted(
      _TEMPERATURE_INTERVALS_L[ion_symbol], T_e_ev
  )
  LZ_coeffs_in_range = jnp.take(
      _MAVRIN_2017_L_COEFFS[ion_symbol], interval_indices, axis=1
  )

  X = jnp.log10(T_e_ev)
  Y = jnp.log10(ne_tau / 1e19)

  # 2D polynomial from Mavrin 2017, Eq. 8
  log10_LZ = (
      LZ_coeffs_in_range[0]
      + LZ_coeffs_in_range[1] * X
      + LZ_coeffs_in_range[2] * Y
      + LZ_coeffs_in_range[3] * X**2
      + LZ_coeffs_in_range[4] * X * Y
      + LZ_coeffs_in_range[5] * Y**2
      + LZ_coeffs_in_range[6] * X**3
      + LZ_coeffs_in_range[7] * X**2 * Y
      + LZ_coeffs_in_range[8] * X * Y**2
      + LZ_coeffs_in_range[9] * Y**3
  )
