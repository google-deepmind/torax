...
class ToraxConfig(torax_pydantic.BaseModelFrozen):
  ...
  def _check_fields(self) -> typing_extensions.Self:
    ...
    if (
        ...
        and (
            using_linear_solver
            or initial_guess_mode == enums.InitialGuessMode.LINEAR
        )
        and not self.stepper.stepper_config.use_pereverzev
    ):
      logging.warning("""
          use_pereverzev=False in a configuration where setting use_pereverzev=True
          is recommended.
          ...
          With this configuration, it is strongly recommended to set use_pereverzev=True
          to avoid numerical instability in the linear solver.
          """)
    return self
