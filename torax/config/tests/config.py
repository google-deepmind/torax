...
from torax.config import config
...
class ConfigTest(parameterized.TestCase):
  ...
  def test_full_config_construction(self):
    ...
    module_config = {key: module.CONFIG[key] for key in config.ToraxConfig}
    config_pydantic = config.ToraxConfig.from_dict(module_config)
    ...
    with self.subTest("json_serialization"):
      ...
      config_pydantic_roundtrip = config.ToraxConfig.model_validate_json(
          config_json
      )
