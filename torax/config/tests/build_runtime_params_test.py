...
class RuntimeParamsSliceTest(parameterized.TestCase):
  ...
  def test_boundary_conditions_are_time_dependent(self):
    ...
    profile_conditions = profile_conditions_lib.ProfileConditions(
        ...
        n_e_bound_right=({5.0: 6.0, 7.0: 8.0}, 'step'),
    )
  ...
  def test_profile_conditions_set_electron_density_and_boundary_condition(
      self,
      n_e_bound_right,
      n_e_bound_right_is_fGW,  # pylint: disable=invalid-name
      n_e_is_fGW,  # pylint: disable=invalid-name
  ):
    """Tests that the profile conditions can set the electron temperature."""
    profile_conditions = profile_conditions_lib.ProfileConditions(
    ...
    if n_e_bound_right is None:
      ...
      self.assertFalse(dynamic_profile_conditions.n_e_bound_right_is_absolute)
    else:
      self.assertEqual(
          dynamic_profile_conditions.n_e_bound_right_is_fGW,
          n_e_bound_right_is_fGW,
      )
      self.assertTrue(dynamic_profile_conditions.n_e_bound_right_is_absolute)
