...
class DynamicProfileConditions:
  ...
  n_e_is_fGW: bool
  n_e_bound_right: array_typing.ScalarFloat
  n_e_bound_right_is_fGW: bool
  n_e_bound_right_is_absolute: bool
  nu: float
...
class ProfileConditions(torax_pydantic.BaseModelFrozen):
  ...
  Attributes:
    I_total: Total plasma current in MA. Note that if Ip_from_parameters=False
    ...
    nbar: Line averaged density. In units of reference density if n_e_is_fGW =
      False. In Greenwald fraction if n_e_is_fGW = True. nGW = Ip/(pi*a^2) with
      a in m, nGW in 10^20 m-3, Ip in MA
    n_e_is_fGW: Toggle units of nbar
    n_e_bound_right: Density boundary condition for r=Rmin. In units of
      reference density if n_e_bound_right_is_fGW = False. In Greenwald fraction
      if `n_e_bound_right_is_fGW = True`. If `n_e_bound_right` is `None` then
      the boundary condition will instead be taken from `n_e` at rhon=1. In this
      case, `n_e_bound_right_is_absolute` will be set to `False` and
      `n_e_bound_right_is_fGW` will be set to `n_e_is_fGW`. If `n_e_bound_right`
      is not `None` then `n_e_bound_right_is_absolute` will be set to `True`.
    n_e_bound_right_is_fGW: Toggle units of n_e_bound_right.
    n_e_bound_right_is_absolute: Toggle units of n_e_bound_right
  ...
  I_total: torax_pydantic.TimeVaryingScalar = torax_pydantic.ValidatedDefault(
      15.0
  )
  ...
  n_e_bound_right: torax_pydantic.TimeVaryingScalar | None = None
  n_e_bound_right_is_fGW: bool = False
  n_e_bound_right_is_absolute: bool = False
  ...
  def after_validator(self) -> Self:
    ...
    if self.n_e_bound_right is None:
  ...
  def build_dynamic_params(
    ...
    if self.T_e_right_bc is None:
      dynamic_params['T_e_right_bc'] = self.Te.get_value(
          t, grid_type='face_right'
      )

    if self.T_i_right_bc is None:
      dynamic_params['T_i_right_bc'] = self.Ti.get_value(
          t, grid_type='face_right'
      )

    if self.n_e_bound_right is None:
      dynamic_params['n_e_bound_right'] = self.n_e.get_value(
          t, grid_type='face_right'
      )
      dynamic_params['n_e_bound_right_is_absolute'] = False
      dynamic_params['n_e_bound_right_is_fGW'] = self.n_e_is_fGW
    else:
      dynamic_params['n_e_bound_right_is_absolute'] = True
