  ...
  * It is still necessary to think carefully when making each new class that is
    a static argument. Common "gotchas" while using ``StaticDataclass`` include:
      ...
      * Functions as fields: these must be marked in the metadata as allowed to
        ...
        ``self`` bound to it, one must install ``<class>.foo`` as a field instead.
      * If there are any complications at all, such as using a field that is
        not a primitive or a StaticDataclass, it is probably a good idea to write
        a unit test that constructing two equivalent copies of the class
...
* For pytree arguments, we prefer not to use ``chex.dataclass`` but rather to apply
  ...
  allows us to specify individual fields as static.
