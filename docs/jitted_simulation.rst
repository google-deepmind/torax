...
``jax.jit`` context. This can be useful for performance as well as making use
of other JAX functionality such as batching, automatic differentation and
using accelerators such as GPU or TPU.
...
.. code-block:: python
  ...
  _, _, _, step_fn = experimental.prepare_simulation(torax_config)
  # The simulation loop will exit after executing at most this many time steps.
  max_steps = 100
  sim_states, post_processed_outputs, final_i = experimental.run_loop_jit(
      step_fn=step_fn,
      max_steps=max_steps,
  )
...
.. code-block:: python
  ...
  ip_update = experimental.TimeVaryingScalarReplace(
      value=new_ip_value,
  )
  T_e_update = experimental.TimeVaryingArrayReplace(
