...
The model distinguishes between "fixed" background impurities and "seeded"
control impurities.

Fixed vs. Seeded
^^^^^^^^^^^^^^^^
*   **Fixed Impurities:** Background species (e.g., Helium ash, intrinsic
    Tungsten) defined via ``fixed_impurity_concentrations``.
*   **Seeded Impurities:** Species actively injected for control
    (e.g., Neon, Argon) defined via ``seed_impurity_weights``. Used only in
    **Inverse Mode**.
...
*   **Automatic:** Set ``use_enrichment_model`` to ``True`` to calculate
    :math:`E` dynamically using the Kallenbach scaling |kallenbach2024|. For
...
``use_enrichment_model`` (bool [default = True])
  If True, calculate enrichment factors using the Kallenbach model. In limited
  configurations, model output will be 1.0 for each species.

``enrichment_model_multiplier`` (**time-varying-scalar** [default = 1.0])
  Multiplier applied to the calculated Kallenbach enrichment factor.
...
``ratio_bpol_omp_to_bpol_avg`` (**time-varying-scalar** | None [default = None])
  Ratio of poloidal field at OMP to the separatrix average.
