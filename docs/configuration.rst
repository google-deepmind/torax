...
loader function ``load_imas_data`` from |imas_loader|.
It can then be loaded programatically in the ``CONFIG`` by constructing a nested
dictionary with the ``core_profiles_from_imas`` function from |core_profiles_input_imas|. The function returns 
...
Example on how to inject the IMAS conditions into the config can be found in
