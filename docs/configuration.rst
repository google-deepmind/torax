...
``current_profile_nu`` (float [default = 1.0])
  ...
  :math:`j = j_0(1 - r^2/a^2)^{\text{current\_profile\_nu}}`. Used if
...
``rotation_multiplier`` (float [default = 1.0])
  Multiplier for :math:`v_{ExB}` in the rotation correction factor.
...
``rotation_multiplier`` (float [default = 1.0])
  Multiplier for :math:`v_{ExB}\_{\text{shear}}`.

``use_rotation`` (bool [default = False])
  If ``True``, use the rotation term :math:`v_{ExB}\_{\text{shear}}` in the model.
